...
public:
    ...
    //same as above but for bouncing photons
    Color3f Sample_f_photon(const Vector3f &woW, Vector3f *wiW, const Point2f &xi, const float newX,
                     float *pdf, BxDFType type = BSDF_ALL,
                     BxDFType *sampledType = nullptr) const;
...
public:
    ...
    bool MatchesFlags(BxDFType t) const {
        BxDFType result = BxDFType(type & t);
//        if ( result == type ) {
        if ( result != 0 ) {
            return true;
        } else {
