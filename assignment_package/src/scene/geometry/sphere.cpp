...
Bounds3f Sphere::WorldBound() const {
    Bounds3f modelbounds(Point3f(-1.f, -1.f, -1.f), Point3f(1.f, 1.f, 1.f));
    return modelbounds.Apply(this->transform);
}

void Sphere::getSurfaceEmissionRay(const Point2f &xi, const Point2f& xk, Ray& ray ,float& pdf) const {
    pdf = 1.f / Area();

    Point3f surfpoint = WarpFunctions::squareToSphereUniform(xi);
    surfpoint += (0.005f *  surfpoint);
